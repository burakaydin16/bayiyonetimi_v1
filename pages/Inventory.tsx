                    ...
                    <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl flex flex-col max-h-[90vh]">
                        ...
                        <div className="p-6 overflow-y-auto space-y-4 custom-scrollbar">
                            ...
                            <div>
                                ...
                                <select
                                    className="w-full border border-gray-200 rounded-lg p-3 focus:ring-2 focus:ring-indigo-500 outline-none bg-white font-medium"
                                    value={formData.type}
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        type="number"
                                        className="w-full border border-gray-200 rounded-lg p-3 pl-8 focus:ring-2 focus:ring-indigo-500 outline-none font-bold"
                            ...
                            <div>
                                ...
                                <input
                                    type="number"
                                    className={`w-full border border-gray-200 rounded-lg p-3 outline-none font-bold ${editingProduct ? 'bg-gray-50 text-gray-400 cursor-not-allowed' : 'focus:ring-2 focus:ring-indigo-500'}`}
                                    value={formData.stock}
                                    onChange={e => setFormData({ ...formData, stock: Number(e.target.value) })}
                                    disabled={!!editingProduct}
                                />
                                {editingProduct && (
                                    <p className="text-[11px] text-orange-600 flex items-start mt-2 bg-orange-50/50 p-2.5 rounded-lg border border-orange-100">
                                        <AlertCircle size={14} className="mr-2 shrink-0 mt-0.5" />
                                        <span>Stok hareketlerini "Hareket Ekle" menüsünden satış veya giriş yaparak yönetin.</span>
                                    </p>
                                )}
                            </div>
                            ...
                            {formData.type === ProductType.WATER && (
                                <div className="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100 space-y-2">
                                    <label className="block text-[13px] font-bold text-indigo-900 uppercase tracking-wide">Bağlı Depozito Ürünü</label>
                                    <select
                                        className="w-full border border-indigo-200 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-white font-medium shadow-sm transition-all"
                                        value={formData.linked_deposit_id || ''}
                                        onChange={e => setFormData({ ...formData, linked_deposit_id: e.target.value })}
                                    >
                                        <option value="">Depozito Bağlantısı Yok (Tek Kullanımlık)</option>
                                        {products.filter(p => p.type === ProductType.DEPOSIT).map(p => (
                                            <option key={p.id} value={p.id}>{p.name}</option>
                                        ))}
                                    </select>
                                    <p className="text-[11px] text-indigo-500 leading-relaxed">
                                        Bu ürün satıldığında, müşterinin hesabına otomatik olarak bu boş ürünün borcu (iade yükümlülüğü) yansıtılır.
                                    </p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex justify-end gap-3 p-6 border-t shrink-0 bg-gray-50/50 rounded-b-2xl">
                            ...
                            <Button onClick={handleSave} className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 shadow-md">Kaydet</Button>
                        </div>
                    </div>
                </div>
            )}
